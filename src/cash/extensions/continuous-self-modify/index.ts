 ...
 *
 * When CASH_CONTINUOUS_SELF_MODIFY=true, creates recurring SELF_MODIFY tasks
 * so the system runs upgrade/validate cycles until the human says stop
 * (cash.pause or CASH_KILL_SWITCH).
 *
 * FITNESS ASSESSMENT INTEGRATION (README §0.4 - MANDATORY FIRMWARE):
 * This extension includes fitness re-assessment tasks in its cycle.
 * All modules, including "stable" ones, are re-assessed periodically (default: weekly).
 * Fitness tasks are created alongside SELF_MODIFY tasks in scheduleNextCycle().
 */
...
export class ContinuousSelfModifyExtension implements ExtensionLifecycle {
  ...
  async initialize(foundation: ExtensionFoundation): Promise<void> {
    ...
    if (process.env.CASH_CONTINUOUS_SELF_MODIFY === "true") {
  ...
  private async scheduleNextCycle(): Promise<void> {
    ...
    if (!f) return;
    // Only stop scheduling if explicitly paused (kill switch / cash.pause).
  ...
  async execute(
    ...
    if (isContinuousCycle) {
      ...
      try {
        ...
        const prompt = `You are the CASH ${task.personaOwner} persona performing a self-improvement cycle.
...
Pick ONE file from src/cash/ or src/agents/ and make a concrete improvement:
...
${context.identityContext}`;
        ...
        const replyText = await runAgentStep({
          sessionKey,
          message: prompt,
          extraSystemPrompt: `You are CASH ${task.personaOwner} persona. ACTION OVER ANALYSIS. Make changes immediately, don't analyze. Read one file max, then edit it.`,
