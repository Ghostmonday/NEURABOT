 ...
 * - The Brain (§6): Creates and manages sub-tasks via CASH
 ...
 * This extension creates fitness assessment tasks for roadmap track modules.
 * Fitness tasks are created alongside roadmap sub-tasks in the executor.
 */
...
const MAX_SUBTASKS_PER_EXECUTION = process.env.CASH_FAST_MODE === "true" ? 20 : 10;
const FOLLOW_UP_INTERVAL_MS =
  process.env.CASH_FAST_MODE === "true" ? 15 * 60 * 1000 : 60 * 60 * 1000; // 15 min fast vs 1 hour
...
class RoadmapObserverExecutor implements PersonaExecutor {
  ...
  async execute(
    ...
    try {
      ...
      // Ensure README.md exists (compute path dynamically for tests)
      const readmePath = getReadmePath();
      if (!fs.existsSync(readmePath)) {
        return {
          success: false,
          outcome: "READ_ERROR",
          summary: `README.md not found at ${readmePath}`,
          confidence: 1.0,
