...
const MiniMaxConfigSchema = z.object({
  apiKey: z.string().regex(/^sk-cp-[A-Za-z0-9_-]{40,}$/, "Invalid MiniMax API key format (must start with sk-cp-)"),
});
...
export function validateSowwyEnv(): ValidatedEnv {
  ...
  if (minimaxApiKey) {
    ...
    if (!result.success) {
      errors.push(`MINIMAX_API_KEY validation failed: ${result.error.errors[0].message}`);
    } else {
      minimax = result.data;
    }
  }
  ...
  if (!minimax) {
    throw new Error(`Sowwy environment validation failed:\n${errors.map(e => `  - ${e}`).join("\n")}`);
  }
  ...
  if (pgHost) {
    ...
    if (!pgPort || !pgUser || !pgPassword || !pgDb) {
      errors.push("If SOWWY_POSTGRES_HOST is set, all PostgreSQL vars are required: SOWWY_POSTGRES_PORT, SOWWY_POSTGRES_USER, SOWWY_POSTGRES_PASSWORD, SOWWY_POSTGRES_DB");
    } else {
      ...
      if (!result.success) {
        errors.push(`PostgreSQL config validation failed: ${result.error.errors.map(e => e.message).join(", ")}`);
  ...
  if (!smtResult.success) {
    errors.push(`SMT config validation failed: ${smtResult.error.errors.map(e => e.message).join(", ")}`);
  }
  ...
  if (!schedulerResult.success) {
    errors.push(`Scheduler config validation failed: ${schedulerResult.error.errors.map(e => e.message).join(", ")}`);
  }
  ...
  if (!smtResult.success || !schedulerResult.success) {
    throw new Error(`SMT or Scheduler config validation failed:\n${
      smtResult.success ? "" : `  - SMT: ${smtResult.error.errors.map(e => e.message).join(", ")}\n`
    }${
      schedulerResult.success ? "" : `  - Scheduler: ${schedulerResult.error.errors.map(e => e.message).join(", ")}`
    }`);
  }
