...
import { Task, PersonaOwner } from "./schema.js";
...
export class TaskScheduler {
  ...
  private personaExecutors: Map<string, (task: Task, context: string) => Promise<TaskExecutionResult>>;
   ...
   */
  registerPersona(persona: PersonaOwner, executor: (task: Task, context: string) => Promise<TaskExecutionResult>): void {
  ...
  private async tick(): Promise<void> {
    ...
    // Get next ready task
    const task = await this.taskStore.getNextReady();
    if (!task) {
      return;
    }
  ...
  private async executeTask(task: Task, identityContext: string): Promise<void> {
    ...
    try {
      ...
      console.log(`[Scheduler] Task ${redactString(task.taskId)} completed: ${redactString(result.outcome)}`);
  ...
  private async handleTaskFailure(task: Task, error: unknown): Promise<void> {
    ...
    if (newRetryCount >= task.maxRetries) {
      ...
      console.log(`[Scheduler] Task ${redactString(task.taskId)} retry ${newRetryCount}/${task.maxRetries} after ${backoffMs}ms`);
    }
  }
  ...
  private async notifyHuman(task: Task): Promise<void> {
    ...
    console.log(`[Scheduler] Human approval needed for task ${redactString(task.taskId)}: ${redactString(task.title)}`);
      ...
      try {
        ...
        if (stuckTasks.length > 0) {
          ...
          for (const task of stuckTasks) {
            ...
            console.warn(`[Scheduler] Auto-blocked stuck task ${redactString(task.taskId)}: ${redactString(task.title)}`);
          }
        }
      } catch (error) {
